...
RC BplusTreeHandler::insert_into_leaf(PageNum leaf_page, const char *pkey, const RID *rid) {
  ...
  for (insert_pos = 0; insert_pos < node->key_num; insert_pos++) {
    tmp = CmpKey(file_header_.attr_type,
...
bool BplusTreeScanner::satisfy_condition(const char *pkey) {
  ...
  switch (attr_type) {
    case INTS:i1 = *(int *) pkey;
      i2 = *(int *) value_;
      break;
    case FLOATS:f1 = *(float *) pkey;
      f2 = *(float *) value_;
      break;
    case CHARS:s1 = pkey;
      s2 = value_;
      break;
    default:LOG_PANIC("Unknown attr type: %d", attr_type);
  }
  ...
  switch (comp_op_) {
    case EQUAL_TO:
      switch (attr_type) {
        case INTS:flag = (i1 == i2);
          break;
        case FLOATS:flag = 0 == float_compare(f1, f2);
          break;
        case CHARS:flag = (strncmp(s1, s2, attr_length) == 0);
          break;
        default:LOG_PANIC("Unknown attr type: %d", attr_type);
      }
      break;
    ...
    case NOT_EQUAL:
      ...
      break;
    default:LOG_PANIC("Unknown comp op: %d", comp_op_);
  }
  return flag;
}
