  ...
  for (unsigned int i = 0; i < field_values.size(); i++) {
    if (!field_values[i].isString()) {
      LOG_ERROR("Field name of index [%s] is not a string. json value=%s",
                name_value.asCString(), field_values[i].toStyledString().c_str());
      return RC::GENERIC_ERROR;
    }
    ...
    if (nullptr == field) {
      LOG_ERROR("Deserialize index [%s]: no such field: %s",
                name_value.asCString(), field_values[i].asCString());
...
std::vector<std::string> IndexMeta::field() const { return field_; }
