...
RC TableMeta::init(const char* name, int field_num,
  ...
  for (int i = 0; i < field_num; i++) {
    ...
    bool nullable = attr_info.null_type == NULLABLE ? true : false;
    ...
    field_offset += attr_info.length + (nullable ? 1 : 0);
  }
...
const IndexMeta* TableMeta::find_index_by_field(
  ...
  for (const IndexMeta& index : indexes_) {
    std::vector<std::string> index_field = index.field();
    if (index_field.size() != field.size()) {
      continue;
    }
    bool flag = true;
